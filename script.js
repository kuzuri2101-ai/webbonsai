...
function closeSearch() {
  if (!searchModal) return;
  searchModal.classList.remove('open'); searchModal.setAttribute('aria-hidden', 'true'); body.classList.remove('no-scroll');
}
...
if (headerActions && !document.querySelector('.login-link')) {
  ...
  loginLink.addEventListener('mouseenter', () => { loginLink.style.background = 'rgba(157,187,143,.12)'; }); loginLink.addEventListener('mouseleave', () => { loginLink.style.background = 'transparent'; }); headerActions.insertBefore(loginLink, headerActions.firstChild);
}

// Kết nối các điểm khác biệt của sản phẩm: Cấp cứu cây + Bonsai Lab.
const rescueUrl = 'cap-cuu-cay.html';
const labUrl = 'lab.html';
document.querySelectorAll('.diagnosis-card').forEach((card, index) => {
  card.setAttribute('href', rescueUrl);
  if (index === 0) card.setAttribute('aria-label', 'Mở công cụ Cấp cứu cây - Vàng lá');
});
const desktopNav = document.querySelector('.desktop-nav');
if (desktopNav && !desktopNav.querySelector('a[href="cap-cuu-cay.html"]')) {
  const rescueLink = document.createElement('a'); rescueLink.href = rescueUrl; rescueLink.textContent = 'Cấp cứu cây'; desktopNav.appendChild(rescueLink);
  const labLink = document.createElement('a'); labLink.href = labUrl; labLink.textContent = 'Bonsai Lab'; desktopNav.appendChild(labLink);
}

// Ảnh mẫu Unsplash có thể bị gỡ: thay thế và có fallback để không xuất hiện ảnh lỗi.
...
const fallbackSvg = `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 800"><defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#10251d"/><stop offset="1" stop-color="#315f46"/></linearGradient></defs><rect width="1200" height="800" fill="url(#bg)"/><circle cx="600" cy="355" r="150" fill="#dce8d7" opacity="0.12"/><path d="M600 510c-12-120 22-220 110-300-14 104-52 199-110 300Zm0 0c-63-92-139-149-228-171 75 74 145 131 228 171Z" fill="#dce8d7" opacity="0.78"/><text x="600" y="625" text-anchor="middle" fill="#f7f3e9" font-family="Arial,sans-serif" font-size="42" font-weight="700">Cây Sao Rồi?</text><text x="600" y="674" text-anchor="middle" fill="#c7d4ca" font-family="Arial,sans-serif" font-size="24">Hình ảnh đang được cập nhật</text></svg>
`)}`;
